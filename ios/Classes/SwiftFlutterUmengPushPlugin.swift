...
public class SwiftFlutterUmengPushPlugin: NSObject, FlutterPlugin {
  ...
  public static func register(with registrar: FlutterPluginRegistrar) {
    channel = FlutterMethodChannel(name: "flutter_umeng_push", binaryMessenger: registrar.messenger())
    let instance = SwiftFlutterUmengPushPlugin()
  ...
  public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
    ...
    public func register(result: @escaping FlutterResult) {
        ...
        // 点击推送启动App，回调监听事件
        if(SwiftFlutterUmengPushPlugin.channel != nil) {
            SwiftFlutterUmengPushPlugin.channel?.invokeMethod("onOpenNotification", arguments: initNotificationUserInfo)
        }
    ...
    public func application(_ application: UIApplication, didReceiveRemoteNotification userInfo: [AnyHashable : Any], fetchCompletionHandler completionHandler: @escaping (UIBackgroundFetchResult) -> Void) -> Bool {
        if(SwiftFlutterUmengPushPlugin.channel != nil) {
            SwiftFlutterUmengPushPlugin.channel?.invokeMethod("onOpenNotification", arguments: groupInvokeMethodArgs(userInfo: userInfo))
        }
        return true;
    }
    ...
    public func userNotificationCenter(_ center: UNUserNotificationCenter, willPresent notification: UNNotification, withCompletionHandler completionHandler: @escaping (UNNotificationPresentationOptions) -> Void) {
        ...
        if(SwiftFlutterUmengPushPlugin.channel != nil) {
            SwiftFlutterUmengPushPlugin.channel?.invokeMethod("onReceiveNotification", arguments: groupInvokeMethodArgs(userInfo: userInfo))
        }
    }
